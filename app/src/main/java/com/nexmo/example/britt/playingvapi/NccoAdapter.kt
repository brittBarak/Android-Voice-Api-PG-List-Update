...
class NccoAdapter(val actions: ListLiveData<NccoAction>) : RecyclerView.Adapter<NccoCompVH>(), RemoveItem {

    override fun onRemoveItem(position: Int) {
        actions.removeItemAt(position)
    }
    ...
    override fun getItemViewType(position: Int): Int {
        return actions[position]?.let { it.actionType.ordinal} ?: -1
