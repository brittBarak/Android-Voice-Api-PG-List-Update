...
data class Talk(val text: String, val voiceName: String? = null, val bargeIn: Boolean? = null, val loop: Int? = null, val level: Double? = null) : NccoAction(ActionType.talk, synchronous = bargeIn != true) //if bargeIn == true then addItem input action
